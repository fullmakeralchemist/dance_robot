...
#include <tinymlinteractivespaces_inferencing.h>
...
void setup()
{
    ...
    BLE.setLocalName("LALO_NICLA_TEST");
...
void loop()
{
    ...
    gesturesave = gesture.c_str();
    ei_printf("\nMax value: %f\n", maxValue);
    if(central.connected()) {
      ei_printf("\nMax value: %f\n", maxValue);
      //if
      if(gesturesave=="jump"){
        gestureDataCharacteristic.writeValue(1);
        Serial.print(1);
      }
      if(gesturesave=="run"){
        Serial.print(2);
        gestureDataCharacteristic.writeValue(2);
      }
      if(gesturesave=="side"){
        Serial.print(3);
        gestureDataCharacteristic.writeValue(3);
      }    
    }
    else {
