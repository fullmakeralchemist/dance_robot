...
class MotorController:
    """Controls the motors for the robotic arms."""
    ...
    def combo(self):
        """Moves both arms in a combo motion."""
        self._move_arms(speed_right=-100, speed_left=100)

    def back(self):
        """Moves both arms back to the initial position."""
        self._move_arms(speed_right=100, speed_left=-100)

    def both(self):
        """Moves both arms at the same time."""
        self._move_arms(speed_right=-100, speed_left=100, blocking=False)

    def backb(self):
        """Moves both arms back at the same time."""
        self._move_arms(speed_right=100, speed_left=-100, blocking=False)

    def _move_arms(self, speed_right, speed_left, duration=1.3, blocking=True):
        """Executes the arm movement for a specified duration."""
...
class SpotifyController:
    ...
    def start_playback(self, device_id, uri):
        """Starts playback of a specified track on Spotify."""
...
class VideoPlayer:
    ...
    def play(self):
        """Plays the video and sets it to fullscreen."""
        self.player.play()
        self.player.set_fullscreen(True)

    def is_finished(self):
        """Checks if the video playback has finished."""
...
def handle_movement(move, mqtt_client, spotify_controller, motor_controller):
    """Handles movements based on the received value."""
    if move == 1:
...
def wait_for_video(video):
    """Waits until the video playback is finished."""
...
class MyDelegate(btle.DefaultDelegate):
    """Handles notifications from the Bluetooth peripheral."""
    ...
    def handleNotification(self, cHandle, data):
        """Processes incoming Bluetooth notifications."""
